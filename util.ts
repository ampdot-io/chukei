...
export async function downloadFile(url: string, destPath: string) {
    if (await hasCommand("aria2c")) {
        console.log("Downloading with aria2c");
        const command = new Deno.Command("aria2c", {
            args: ["-x", "16", "-s", "16", "-d", path.dirname(destPath), "-o", path.basename(destPath), url],
        });
        const { success } = await command.output();
        if (success) return;
        console.log("aria2c failed, falling back...");
    }

    if (await hasCommand("wget")) {
        console.log("Downloading with wget");
        const command = new Deno.Command("wget", {
            args: ["-O", destPath, url],
        });
        const { success } = await command.output();
        if (success) return;
        console.log("wget failed, falling back...");
    }

    console.log("Downloading with Deno fetch");
...
export function securePath(baseDir: string, userPath: string): string | null {
    ...
    const fullPath = path.resolve(resolvedBase, userPath);

    // make sure we're still in the base directory
    if (!fullPath.startsWith(resolvedBase + path.SEPARATOR) && fullPath !== resolvedBase) {
